/*
  # Add Profile Tables (Safe Migration)
...
BEGIN
  -- Profiles trigger
  IF NOT EXISTS (SELECT FROM pg_trigger WHERE tgname = 'set_profiles_updated_at') THEN
  ...
  -- Resumes trigger
  IF NOT EXISTS (SELECT FROM pg_trigger WHERE tgname = 'set_resumes_updated_at') THEN
  ...
  IF NOT EXISTS (SELECT FROM pg_trigger WHERE tgname = 'set_job_descriptions_updated_at') THEN
